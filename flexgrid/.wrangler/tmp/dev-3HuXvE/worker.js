...
var HELIUS_RPC_BASE = "https://mainnet.helius-rpc.com/";
...
function getHeliusApiKey(env2) {
  const k = env2?.HELIUS_API_KEY;
  return k && typeof k === "string" ? k.trim() : "";
}
__name(getHeliusApiKey, "getHeliusApiKey");
function heliusRpcUrl(env2) {
  const k = getHeliusApiKey(env2);
  if (!k) return "";
  return `${HELIUS_RPC_BASE}?api-key=${encodeURIComponent(k)}`;
}
__name(heliusRpcUrl, "heliusRpcUrl");
var IPFS_GATEWAYS = [
...
async function handleApiSolanaNfts(request, env2) {
  ...
  __name(shortenAddress, "shortenAddress");
  function scoreSolanaNFT(asset) {
    ...
    const groupingCollection = Array.isArray(asset?.grouping) ? asset.grouping.find((g) => g && g.group_key === "collection") : null;
  ...
  __name(getSolanaCollectionKey, "getSolanaCollectionKey");
  ...
  __name(getSolanaCollectionName, "getSolanaCollectionName");
  ...
  __name(collectionSignature, "collectionSignature");
  function getImageCandidates(url2) {
    const cleaned = cleanImage(url2);
    if (!cleaned) return [];
  ...
  __name(getImageCandidates, "getImageCandidates");
  function getSolanaCollectionLogoCandidate(asset) {
    const c = asset?.content?.metadata?.collection;
    const maybe = c?.image || c?.imageUrl || c?.image_url || c?.logo || null;
  ...
  __name(getSolanaCollectionLogoCandidate, "getSolanaCollectionLogoCandidate");
  const controller = new AbortController();
  const tid = setTimeout(() => controller.abort(), 25e3);
  try {
    const body = {
      jsonrpc: "2.0",
      id: "flexgrid",
    ...
    const scoredAssets = rows.filter(hasBasicNFTData).map((asset) => ({ asset, score: scoreSolanaNFT(asset) })).filter((x) => x.score >= MIN_NFT_SCORE);
    const cleanAssets = scoredAssets.map((x) => x.asset);
    const collectionsMap = /* @__PURE__ */ new Map();
    const sigToPrimaryKey = /* @__PURE__ */ new Map();
    ...
    const finalCollections = Array.from(collectionsMap.values()).filter((c) => (c.items?.length || 0) >= MIN_COLLECTION_SIZE).sort((a, b) => {
    ...
    const flat = trimmedCollections.flatMap(
      (c) => (c.items || []).map((it) => ({
        ...it,
...
__name(handleApiSolanaNfts, "handleApiSolanaNfts");
...
var worker_default = {
  async fetch(request, env2, ctx) {
    ...
    const hasHeliusKey = !!getHeliusApiKey(env2);
    ...
    if (isConfigPath && request.method === "GET") {
      if (!hasMainKey && !hasMoralisKey && !hasHeliusKey) {
        return corsResponse(
          JSON.stringify({
            error: "This Worker has no API keys. Set ALCHEMY_API_KEY (ETH/Base/Polygon), MORALIS_API_KEY (ApeChain), and/or HELIUS_API_KEY (Solana). For local dev: wrangler secret put \u2026"
          }),
          503
        );
      }
    ...
    if (url.pathname === "/api/solana-nfts" && request.method === "GET") {
      return handleApiSolanaNfts(request, env2);
    }
...
// .wrangler/tmp/bundle-6inKG2/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6inKG2/middleware-loader.entry.ts
