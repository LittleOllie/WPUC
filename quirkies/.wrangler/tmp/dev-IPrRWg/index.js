...
__name(pickBestDisplayableImageUrl, "pickBestDisplayableImageUrl");
...
__name(parseMetadataObjectMaybe, "parseMetadataObjectMaybe");
function alchemyNftMetadataRoot(data) {
  if (!data || typeof data !== "object") return data;
  if (data.nft && typeof data.nft === "object") return data.nft;
  return data;
}
__name(alchemyNftMetadataRoot, "alchemyNftMetadataRoot");
...
var IMG_PROXY_ACCEPT = "image/avif,image/webp,image/apng,image/*,*/*;q=0.8";
function imgProxyHeadersForUrl(resourceUrl, variant = 0) {
  ...
  if (v === 3) {
    return {
      "User-Agent": "Mozilla/5.0 (compatible; Quirks-ImageProxy/1.0; +https://opensea.io/)",
      Accept: IMG_PROXY_ACCEPT
    };
  }
  if (/seadn\.io|looksrare|cdn\.blur\.io|openseauserdata\.com|arweave\.net|cloudinary\.com|googleusercontent\.com|storage\.googleapis\.com|nftstorage\.link|ipfs\.|pinata|dweb\.link|w3s\.link|alchemy\.com|manifoldcdn|lens\.xyz/i.test(
    u
  )) {
    return {
      "User-Agent": IMG_PROXY_UA,
...
async function fetchWithImgTimeout(resource, timeoutMs, headerVariant) {
  ...
  const hv = headerVariant === void 0 ? 0 : headerVariant;
  try {
    ...
    try {
      ...
      if (st === 429 || st === 503) {
        ...
        if (ra && /^\d+$/.test(ra.trim())) {
          extra = Math.min(8e3, parseInt(ra.trim(), 10) * 1e3);
        }
...
// .wrangler/tmp/bundle-aLJRkP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aLJRkP/middleware-loader.entry.ts
