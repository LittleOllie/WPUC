...
async function fetchGateway(contract, id) {
...
const { slug, contract } = parseArgs(process.argv.slice(2));
if (!slug || !contract) {
  console.error("Usage: node patch-collection-images.mjs --slug ogenies --contract 0x...");
  process.exit(1);
}
...
await mapPool(ids, async (id) => {
  const gateway = await fetchGateway(contract, id);
